...
class TimeSpan {
	...
	get days() {
		if (this._milliseconds >= 0)
			...
			return Math.ceil(this.totalDays);
	}
	...
	get hours() {
		if (this._milliseconds >= 0)
			...
			return Math.ceil(this.totalHours) % DAY_TO_HOUR;
	}
	...
	get minutes() {
		if (this._milliseconds >= 0)
			...
			return Math.ceil(this.totalMinutes) % HOUR_TO_MINUTE;
	}
	...
	get seconds() {
		if (this._milliseconds >= 0)
			...
			return Math.ceil(this.totalSeconds) % MINUTE_TO_SECOND;
	}
	...
	get milliseconds() {
		if (this._milliseconds >= 0)
			...
			return Math.ceil(this._milliseconds) % SECOND_TO_MILLISECOND;
	}
	...
	format(formatStr = "hh:mm:ss") {
		...
		for (let key in o) {
			...
			function replacefun() {
				...
				if (numberStr.length > currentRegStr.length)
					return numberStr;
				else
					return currentRegStr.substr(0, currentRegStr.length - numberStr.length) + numberStr;
			}
		}
		return result;
	}
